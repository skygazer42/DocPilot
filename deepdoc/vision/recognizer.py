...
class Recognizer:
    def __init__(self, label_list, task_name, model_dir=None, device_id: int | None = None):
        """
        ...
        self._supports_batched_onnx_inference = self._infer_batched_onnx_support()
        self.label_list = label_list

    def _infer_batched_onnx_support(self) -> bool:
        try:
            input_nodes = self.ort_sess.get_inputs()
            if not input_nodes:
                return True
            shape = list(getattr(input_nodes[0], "shape", []) or [])
            if not shape:
                return True
            batch_dim = shape[0]
    ...
    def close(self):
        logger.info("Close recognizer.")
        if hasattr(self, "ort_sess"):
            del self.ort_sess
        gc.collect()

    def _build_batched_input_dict(self, inputs):
        if not inputs:
            return None
        batched_inputs = {}
        for input_name in self.input_names:
            values = []
            for ins in inputs:
                if input_name not in ins:
                    return None
                value = ins[input_name]
                if not isinstance(value, np.ndarray) or value.ndim == 0:
                    return None
                values.append(value)
            try:
                batched_inputs[input_name] = np.concatenate(values, axis=0)
            except Exception:
                return None
        batch_size = len(inputs)
        for value in batched_inputs.values():
            if value.shape[0] != batch_size:
                return None
        return batched_inputs

    @staticmethod
    def _split_batched_outputs(outputs, batch_size):
        split_outputs = []
        for batch_index in range(batch_size):
            page_outputs = []
            for output in outputs:
                if not isinstance(output, np.ndarray) or output.shape[0] != batch_size:
                    return None
                page_outputs.append(output[batch_index])
            split_outputs.append(page_outputs)
        return split_outputs

    def _run_batch_or_single(self, inputs, thr):
        batched_input_dict = None
        if getattr(self, "_supports_batched_onnx_inference", True):
            batched_input_dict = self._build_batched_input_dict(inputs)
        if batched_input_dict is not None and len(inputs) > 1:
            try:
                outputs = self.ort_sess.run(None, batched_input_dict, self.run_options)
                split_outputs = self._split_batched_outputs(outputs, len(inputs))
                if split_outputs is not None:
                    return [self.postprocess(page_outputs[0], inputs[index], thr) for index, page_outputs in enumerate(split_outputs)]
            except Exception:
                logger.exception("Batched recognizer inference failed; falling back to single-page inference.")

        results = []
        for ins in inputs:
            input_dict = {k: v for k, v in ins.items() if k in self.input_names}
            outputs = self.ort_sess.run(None, input_dict, self.run_options)
            results.append(self.postprocess(outputs[0], ins, thr))
        return results
    ...
    def __call__(self, image_list, thr=0.7, batch_size=16):
        ...
        for i in range(batch_loop_cnt):
            ...
            res.extend(self._run_batch_or_single(inputs, thr))
