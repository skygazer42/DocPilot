...
class TableStructureRecognizer(Recognizer):
    ...
    def __init__(self, device_id: int | None = None):
        super().__init__(self.labels, "tsr", ensure_groups("core"), device_id=device_id)
    ...
    def __call__(self, images, thr=0.2):
        ...
        if table_structure_recognizer_type == "onnx":
            logger.debug("Using Onnx table structure recognizer")
            tbls = super().__call__(images, thr)
        else:  # ascend
            logger.debug("Using Ascend table structure recognizer")
    ...
    def blockType(b):
        ...
        for p, n in patt:
            if re.search(p, b["text"].strip()):
                return n
        tks = [t for t in tokenizer.tokenize(b["text"]).split() if len(t) > 1]
        if len(tks) > 3:
            if len(tks) < 12:
                return "Tx"
            else:
        ...
        if len(tks) == 1 and tokenizer.tag(tks[0]) == "nr":
            return "Nr"
    ...
    def __desc_table(cap, hdr_rowno, tbl, is_english):
        ...
        logger.debug(f">>>>>>>>>>>>>>>>>{cap}：SIZE:{rowno}X{clmno} Header: {hdr_rowno}")
        row_txt = []
        for i in range(rowno):
            if i in hdr_rowno:
                continue
