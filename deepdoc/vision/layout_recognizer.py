...
from common.model_store import ensure_groups, get_model_root
...
class LayoutRecognizer(Recognizer):
    ...
    def __init__(self, domain, device_id: int | None = None):
        model_dir = ensure_groups("core")
        super().__init__(self.labels, domain, model_dir, device_id=device_id)
...
class LayoutRecognizer4YOLOv10(LayoutRecognizer):
    ...
    def __init__(self, domain, device_id: int | None = None):
        del domain
        super().__init__("layout", device_id=device_id)
        self.auto = False
...
class PPDocLayoutRecognizer(LayoutRecognizer):
    """PP-DocLayout-plus/RT-DETR layout recognizer.

    This backend is deliberately separate from the YOLOv10 layout branch: the
    output contract is RT-DETR-style ``[x1, y1, x2, y2, score, cls]`` rows, and
    the 23 Paddle layout classes are mapped back to DeepDoc's existing layout
    type vocabulary so downstream parser logic remains unchanged.
    """
    ...
    def __init__(self, domain=None, device_id: int | None = None):
        model_dir = ensure_groups("layout_v2")
        Recognizer.__init__(self, self.labels, "layout/pp_doclayout_plus", model_dir, device_id=device_id)
        self.garbage_layouts = ["footer", "header", "reference"]
        self.auto = False
        self.scaleFill = False
        self.scaleup = True
    ...
    def _map_label(self, class_id: int) -> str | None:
        if class_id < 0 or class_id >= len(self.label_list):
            return None
        raw_label = str(self.label_list[class_id] or "").strip().lower()
        return self.label_aliases.get(raw_label, raw_label)
    ...
    def preprocess(self, image_list):
        ...
        for img in image_list:
            ...
            bottom = int(round(dh + 0.1)) if self.center else 0
            left = int(round(dw - 0.1)) if self.center else 0
            right = int(round(dw + 0.1)) if self.center else 0
            img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114))
            img /= 255.0
            img = img.transpose(2, 0, 1)
            img = img[np.newaxis, :, :, :].astype(np.float32)
            inputs.append({self.input_names[0]: img, "scale_factor": [shape[1] / ww, shape[0] / hh], "pad": [dw, dh]})
    ...
    def postprocess(self, boxes, inputs, thr):
        ...
        if arr.ndim == 1:
            arr = arr.reshape(1, -1)
        if arr.shape[1] < 6:
            raise ValueError(f"Unexpected PP-DocLayout output shape: {arr.shape}")
        ...
        boxes = np.multiply(boxes, np.array([sx, sy, sx, sy], dtype=np.float32), dtype=np.float32)
        ...
        results = []
        for i in indices:
            mapped_type = self._map_label(int(class_ids[i]))
            if not mapped_type:
                continue
            results.append(
                {
