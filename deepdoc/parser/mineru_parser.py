...
class MinerUBackend(StrEnum):
    ...
    VLM_VLLM_ASYNC_ENGINE = (
        "vlm-vllm-async-engine"  # Asynchronous vLLM engine, new in MinerU API
    )
...
class MinerUParser(DeepDocPdfParser):
    def __init__(
        self,
    ...
    def _extract_zip_no_root(self, zip_path, extract_to, root_dir):
        logger.info(
            f"[MinerU] Extract zip: zip_path={zip_path}, extract_to={extract_to}, root_hint={root_dir}"
        )
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            ...
            for member in members:
                if member.flag_bits & 0x1:
                    raise RuntimeError(
                        f"[MinerU] Encrypted zip entry not supported: {member.filename}"
                    )
                if self._is_zipinfo_symlink(member):
                    raise RuntimeError(
                        f"[MinerU] Symlink zip entry not supported: {member.filename}"
                    )

                name = member.filename.replace("\\", "/")
                if root_dir and name == root_dir:
                    logger.info("[MinerU] Ignore root folder...")
                    continue
                ...
                if (
                    ...
                    raise RuntimeError(
                        f"[MinerU] Unsafe zip path (absolute): {member.filename}"
                    )

                parts = [p for p in name.split("/") if p not in ("", ".")]
                if any(p == ".." for p in parts):
                    raise RuntimeError(
                        f"[MinerU] Unsafe zip path (traversal): {member.filename}"
                    )
                ...
                if dest_path != base_dir and base_dir not in dest_path.parents:
                    raise RuntimeError(
                        f"[MinerU] Unsafe zip path (escape): {member.filename}"
                    )
    ...
    def _normalize_backend(backend: str) -> str:
        value = (backend or "pipeline").strip().lower()
        return value

    @staticmethod
    def _normalize_lang_code(lang: str) -> str:
        value = (lang or "Chinese").strip()
        lower = value.lower()
        if lower in {
            "ch",
            "en",
            ...
            return lower
        return LANGUAGE_TO_MINERU_MAP.get(value, "ch")

    def check_installation(
        self, backend: str = "pipeline", server_url: Optional[str] = None
    ) -> tuple[bool, str]:
        reason = ""
        ...
        try:
            ...
            logger.info(
                f"[MinerU] API openapi.json reachable={api_ok} url={api_openapi}"
            )
            if not api_ok:
        ...
        if backend == "vlm-http-client":
            ...
            if not resolved_server:
                reason = (
                    "[MinerU] MINERU_SERVER_URL required for vlm-http-client backend."
                )
                logger.warning(reason)
                return False, reason
            try:
                ...
                logger.info(
                    f"[MinerU] vlm-http-client server check reachable={server_ok} url={resolved_server}"
                )
            except Exception as exc:
                logger.warning(
                    f"[MinerU] vlm-http-client server probe failed: {resolved_server}: {exc}"
                )
    ...
    def _run_mineru_api(
        ...
        api_base = (self.mineru_api or "").strip()
        ...
        if (
            ...
            raise RuntimeError(
                f"[MinerU] invalid api base url: '{self.mineru_api}'. "
                "Set MINERU_APISERVER to http(s)://host:port"
            )
        ...
        output_path = tempfile.mkdtemp(
            prefix=f"{pdf_file_name}_{options.method}_", dir=str(output_dir)
        )
        ...
        logger.info(f"[MinerU] request {data=}")
        logger.info(f"[MinerU] request {options=}")

        headers = {"Accept": "application/json"}
        try:
            endpoints = ["/tasks", "/file_parse"]
            handled = False
            for endpoint in endpoints:
                request_url = f"{api_base}{endpoint}"
                logger.info(
                    f"[MinerU] invoke api: {request_url} backend={options.backend} server_url={data.get('server_url')}"
                )
                if callback:
                    callback(0.20, f"[MinerU] invoke api: {request_url}")
                with open(pdf_file_path, "rb") as pdf_file:
                    files = {
                        "files": (pdf_file_name + ".pdf", pdf_file, "application/pdf")
                    }
                    with requests.post(
                        url=request_url,
                        files=files,
                        ...
                        if response.status_code == 404:
                            logger.warning(
                                f"[MinerU] endpoint not found: {request_url}"
                            )
                            continue
                        ...
                        if content_type.startswith("application/zip"):
                            logger.info(
                                f"[MinerU] zip file returned, saving to {output_zip_path}..."
                            )
                            ...
                            self._extract_zip_no_root(
                                output_zip_path, output_path, pdf_file_name + "/"
                            )
                            ...
                            break
                        logger.warning(
                            f"[MinerU] not zip returned from api: {content_type}"
                        )

            if not handled:
                raise RuntimeError(
                    "[MinerU] no valid zip response from /tasks or /file_parse"
                )
        except Exception as e:
            raise RuntimeError(f"[MinerU] api failed with exception {e}")
        logger.info("[MinerU] Api completed successfully.")
    ...
    def _line_tag(self, bx):
        ...
        if (
            ...
            and len(self.page_images) > bx["page_idx"]
        ):
    ...
    def crop(self, text, ZM=1, need_position=False):
        ...
        if not getattr(self, "page_images", None):
            logger.warning(
                "[MinerU] crop called without page images; skipping image generation."
            )
            if need_position:
                return None, None
            return
        ...
        for pns, left, right, top, bottom in poss:
            if not pns:
                logger.warning(
                    "[MinerU] Empty page index list in crop; skipping this position."
                )
                continue
            valid_pns = [p for p in pns if 0 <= p < page_count]
            if not valid_pns:
                logger.warning(
                    f"[MinerU] All page indices {pns} out of range for {page_count} pages; skipping."
                )
                continue
        ...
        if not poss:
            logger.warning(
                "[MinerU] No valid positions after filtering; skip cropping."
            )
            if need_position:
                return None, None
            return
    ...
    def _read_output(
        ...
        logger.info(
            f"[MinerU] Expected output files: {', '.join(sorted(allowed_names))}"
        )
        logger.info(f"[MinerU] Searching output in: {output_dir}")

        jf = output_dir / f"{file_stem}_content_list.json"
        logger.info(f"[MinerU] Trying original path: {jf}")
        attempted.append(jf)
        if jf.exists():
            subdir = output_dir
            json_file = jf
            ...
            logger.info(f"[MinerU] Trying sanitized filename: {alt}")
            ...
            if alt.exists():
                ...
                logger.info(f"[MinerU] Trying sanitized nested path: {nested_alt}")
        ...
        if not json_file:
            recursive_candidates = list(output_dir.rglob("*_content_list.json"))
            if not recursive_candidates:
                recursive_candidates = list(output_dir.rglob("content_list.json"))
            ...
            if recursive_candidates:
                preferred = [
                    ...
                    or safe_stem in p.name
                    or file_stem in p.name
                ]
                json_file = preferred[0] if preferred else recursive_candidates[0]
                subdir = json_file.parent
                logger.info(f"[MinerU] Fallback matched output file: {json_file}")
            else:
                extracted_files = [
                ...
                logger.warning(
                    f"[MinerU] Extracted files ({len(extracted_files)}): {extracted_files[:200]}"
                )
                raise FileNotFoundError(
                    f"[MinerU] Missing output file, tried: {', '.join(str(p) for p in attempted)}"
                )
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        return_images = bool(parser_cfg.get("mineru_return_images", False))
        start_page_id = int(parser_cfg.get("mineru_start_page_id", 0))
        end_page_id = int(parser_cfg.get("mineru_end_page_id", 99999))
        backend = self._normalize_backend(backend)
        ...
        if binary:
            temp_dir = Path(
                tempfile.mkdtemp(prefix="mineru_bin_pdf_", dir=str(base_tmp_dir))
            )
            temp_pdf = temp_dir / pdf_file_name
            with open(temp_pdf, "wb") as f:
                f.write(binary)
            pdf = temp_pdf
            logger.info(f"[MinerU] Received binary PDF -> {temp_pdf}")
            if callback:
                callback(0.15, f"[MinerU] Received binary PDF -> {temp_pdf}")
        else:
            if pdf_file_path_valid != filepath:
                logger.info(
                    f"[MinerU] Remove all space in file name: {pdf_file_path_valid}"
                )
        ...
        if output_dir:
            ...
            out_dir = Path(
                tempfile.mkdtemp(prefix="mineru_pdf_", dir=str(base_tmp_dir))
            )
            created_tmp_dir = True

        logger.info(
            f"[MinerU] Output directory: {out_dir} backend={backend} api={self.mineru_api} server_url={server_url or self.mineru_server_url}"
        )
        if callback:
        ...
        try:
            ...
            outputs = self._read_output(
                final_out_dir, pdf.stem, method=mineru_method_raw_str, backend=backend
            )
            logger.info(f"[MinerU] Parsed {len(outputs)} blocks from PDF.")
            if callback:
                callback(0.75, f"[MinerU] Parsed {len(outputs)} blocks from PDF.")

            return (
                self._transfer_to_sections(outputs, parse_method),
                self._transfer_to_tables(outputs),
                {
                    "raw_outputs": outputs,
                    "output_dir": str(final_out_dir or out_dir),
                },
            )
        finally:
            ...
            if (
                delete_output
                and isinstance(final_out_dir, Path)
                and final_out_dir.exists()
                and final_out_dir != out_dir
            ):
                try:
                    shutil.rmtree(final_out_dir)
                except Exception:
                    pass
            if delete_output and out_dir.exists() and created_tmp_dir:
                try:
