...
class DoclingParser(DeepDocPdfParser):
    def __init__(self):
    ...
    def check_installation(self) -> bool:
        if DocumentConverter is None:
            logger.warning(
                "[Docling] 'docling' is not importable, please: pip install docling"
            )
            return False
        try:
            _ = DocumentConverter()
            ...
            logger.error(f"[Docling] init DocumentConverter failed: {e}")
            return False
    ...
    def _make_line_tag(self, bbox: _BBox) -> str:
        ...
        if (
            ...
            and len(self.page_images) >= bbox.page_no
        ):
    ...
    def crop(self, text: str, ZM: int = 1, need_position: bool = False):
        ...
        poss.append(
            (
                ...
                pos[1],
                pos[2],
                min(self.page_images[pos[0][-1]].size[1], pos[4] + GAP),
        ...
        for ii, (pns, left, right, top, bottom) in enumerate(poss):
            ...
            for pn in pns[1:]:
                ...
                x0, y0, x1, y1 = (
                    ...
                    0,
                    int(right),
                    int(min(remain_bottom, page.size[1])),
                )
            ...
            if (
                ...
                if getattr(t, "prov", None):
                    ...
                    if pn and bb and len(bb) == 4:
                        bbox = _BBox(
                            page_no=int(pn), x0=bb[0], y0=bb[1], x1=bb[2], y1=bb[3]
                        )
    ...
    def cropout_docling_table(
        self, page_no: int, bbox: tuple[float, float, float, float], zoomin: int = 1
    ):
            ...
            if getattr(tab, "prov", None):
                ...
                if pn is not None and bb is not None:
                    ...
                    if None not in (left, top, right, bott):
                        img, positions = self.cropout_docling_table(
                            int(pn),
    ...
    def parse_pdf(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"[Docling] render pages failed: {e}")
        ...
        if callback:
            callback(
                0.7, f"[Docling] Parsed doc: {getattr(doc, 'num_pages', 'n/a')} pages"
            )
        ...
        if callback:
            callback(
                0.95, f"[Docling] Sections: {len(sections)}, Tables: {len(tables)}"
            )
