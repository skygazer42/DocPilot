...
from common.nlp import find_codec, tokenizer
import uuid
import chardet
...
class DeepDocHtmlParser:
    ...
    def parser_txt(cls, txt, chunk_token_num):
        ...
        cls.read_text_recursively(
            soup.body, temp_sections, chunk_token_num=chunk_token_num
        )
    ...
    def split_table(cls, html_table, chunk_token_num=512):
        ...
        table_str_list = []
        for row in rows:
            tks_str = tokenizer.tokenize(str(row))
        ...
        if isinstance(element, NavigableString):
            ...
            if content:
                if is_valid_html(content):
                    soup = BeautifulSoup(content, "html.parser")
                    child_info = cls.read_text_recursively(
                        soup, parser_result, chunk_token_num, element.name, block_id
                    )
            ...
            if str.lower(element.name) == "table":
                ...
                for child in element.children:
                    child_info = cls.read_text_recursively(
                        child, parser_result, chunk_token_num, element.name, block_id
                    )
                    parser_result.extend(child_info)
        return []
    ...
    def chunk_block(cls, block_txt_list, chunk_token_num=512):
        ...
        for block in block_txt_list:
            tks_str = tokenizer.tokenize(block)
