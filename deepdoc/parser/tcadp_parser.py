    ...
    def reconstruct_document_sse(self, file_type, file_url=None, file_base64=None, file_start_page=1, file_end_page=1000, config=None):
        ...
        try:
            ...
            if file_url:
                ...
                logger.info(f"[TCADP] Using file URL: {file_url}")
            elif file_base64:
                params["FileBase64"] = file_base64
                logger.info(f"[TCADP] Using Base64 data, length: {len(file_base64)} characters")
            else:
            ...
            if isinstance(resp, types.GeneratorType):  # Streaming response
                logger.info("[TCADP] Detected streaming response")
                for event in resp:
                    logger.info(f"[TCADP] Received event: {event}")
                    if event.get('data'):
                        try:
                            data_dict = json.loads(event['data'])
                            logger.info(f"[TCADP] Parsed data: {data_dict}")

                            if data_dict.get('Progress') == "100":
                                parser_result = data_dict
                                logger.info("[TCADP] Document parsing completed!")
                                logger.info(f"[TCADP] Task ID: {data_dict.get('TaskId')}")
                                logger.info(f"[TCADP] Success pages: {data_dict.get('SuccessPageNum')}")
                                logger.info(f"[TCADP] Failed pages: {data_dict.get('FailPageNum')}")

                                # Print failed page information
                                failed_pages = data_dict.get("FailedPages", [])
                                if failed_pages:
                                    logger.warning("[TCADP] Failed parsing pages:")
                                    for page in failed_pages:
                                        logger.warning(f"[TCADP]   Page number: {page.get('PageNumber')}, Error: {page.get('ErrorMsg')}")

                                # Check if there is a download link
                                download_url = data_dict.get("DocumentRecognizeResultUrl")
                                if download_url:
                                    logger.info(f"[TCADP] Got download link: {download_url}")
                                else:
                                    logger.warning("[TCADP] No download link obtained")

                                break  # Found final result, exit loop
                            else:
                                # Print progress information
                                progress = data_dict.get("Progress", "0")
                                logger.info(f"[TCADP] Progress: {progress}%")
                        except json.JSONDecodeError as e:
                            logger.error(f"[TCADP] Failed to parse JSON data: {e}")
                            logger.error(f"[TCADP] Raw data: {event.get('data')}")
                            continue
                    else:
                        logger.info(f"[TCADP] Event without data: {event}")
            else:  # Non-streaming response
                logger.info("[TCADP] Detected non-streaming response")
                if hasattr(resp, 'data') and resp.data:
                    try:
                        data_dict = json.loads(resp.data)
                        parser_result = data_dict
                        logger.info(f"[TCADP] JSON parsing successful: {parser_result}")
                    except json.JSONDecodeError as e:
                        logger.error(f"[TCADP] JSON parsing failed: {e}")
                        return None
                else:
                    logger.error("[TCADP] No data in response")
                    return None

            return parser_result

        except TencentCloudSDKException as err:
            logger.error(f"[TCADP] Tencent Cloud SDK error: {err}")
            return None
        except Exception as e:
            logger.error(f"[TCADP] Unknown error: {e}")
            logger.error(f"[TCADP] Error stack trace: {traceback.format_exc()}")
            return None

    def download_result_file(self, download_url, output_dir):
        """Download parsing result file"""
        if not download_url:
            logger.warning("[TCADP] No downloadable result file")
            return None
        ...
        try:
            ...
            logger.info(f"[TCADP] Document parsing result downloaded to: {os.path.basename(file_path)}")
            return file_path

        except Exception as e:
            logger.error(f"[TCADP] Failed to download file: {e}")
            try:
...
class TCADPParser(DeepDocPdfParser):
    def __init__(self, secret_id: str = None, secret_key: str = None, region: str = "ap-guangzhou",
        ...
        logger.info(f"[TCADP] Initializing with parameters - table_result_type: {table_result_type}, markdown_image_response_type: {markdown_image_response_type}")

        # Priority: read configuration from DeepDoc configuration system (service_conf.yaml)
        try:
            ...
            if isinstance(tcadp_parser, dict) and tcadp_parser:
                ...
                logger.error("[TCADP] Please configure tcadp_config in service_conf.yaml first")
            ...
            logger.info("[TCADP] Configuration module import failed")
        ...
        # Log final values
        logger.info(f"[TCADP] Final values - table_result_type: {self.table_result_type}, markdown_image_response_type: {self.markdown_image_response_type}")
    ...
    def check_installation(self) -> bool:
        ...
        try:
            ...
            if not self.secret_id or not self.secret_key:
                logger.error("[TCADP] Tencent Cloud API configuration incomplete")
                return False

            # Try to create client to verify configuration
            TencentCloudAPIClient(self.secret_id, self.secret_key, self.region)
            logger.info("[TCADP] Tencent Cloud API configuration check passed")
            return True
        except Exception as e:
            logger.error(f"[TCADP] Tencent Cloud API configuration check failed: {e}")
            return False
    ...
    def _extract_content_from_zip(self, zip_path: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[TCADP] Failed to extract ZIP file content: {e}")
        ...
        *,
        ...
        try:
            ...
            if binary:
                ...
                logger.info(f"[TCADP] Received binary PDF -> {os.path.basename(file_path)}")
                if callback:
            ...
            for attempt in range(max_retries):
                try:
                    if attempt > 0:
                        logger.info(f"[TCADP] Retry attempt {attempt + 1}")
                        if callback:
                    ...
                    logger.info(f"[TCADP] API request config - TableResultType: {self.table_result_type}, MarkdownImageResponseType: {self.markdown_image_response_type}")
                    ...
                    if result:
                        logger.info(f"[TCADP] Attempt {attempt + 1} successful")
                        break
                    else:
                        logger.warning(f"[TCADP] Attempt {attempt + 1} failed, result is None")

                except Exception as e:
                    logger.error(f"[TCADP] Attempt {attempt + 1} exception: {e}")
            ...
            logger.info(f"[TCADP] Extracted {len(content_data)} content blocks")
            ...
            logger.info(f"[TCADP] Parsing completed: {len(sections)} sections, {len(tables)} tables")
