...
from common.nlp import surname, tokenizer
...
def forEdu(cv):
    ...
    for ii, n in enumerate(sorted(cv["education_obj"], key=lambda x: x.get("start_time", "3"))):
        ...
        if n.get("school_name") and isinstance(n["school_name"], str):
            ...
            e["sch_nm_kwd"] = sch[-1]
        fea.append(tokenizer.fine_grained_tokenize(tokenizer.tokenize(n.get("school_name", ""))).split()[-1])
...
def forProj(cv):
    ...
    if pro_nms:
        # cv["pro_nms_tks"] = tokenizer.tokenize(" ".join(pro_nms))
