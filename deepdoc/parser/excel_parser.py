...
class DeepDocExcelParser:
    @staticmethod
    def _ensure_file_like(file_input):
    ...
    def _load_excel_to_workbook(file_like_object):
        file_like_object = DeepDocExcelParser._ensure_file_like(file_like_object)
        ...
        if not (
            ...
            try:
                ...
                return DeepDocExcelParser._dataframe_to_workbook(df)

            except Exception as e_csv:
                raise Exception(
                    f"Failed to parse CSV and convert to Excel Workbook: {e_csv}"
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return DeepDocExcelParser._dataframe_to_workbook(dfs)
                except Exception as ex:
                    logger.info(
                        f"pandas with default engine load error: {ex}, try calamine instead"
                    )
                    file_like_object.seek(0)
                ...
                raise Exception(
                    f"pandas.read_excel error: {e_pandas}, original openpyxl error: {e}"
                )
    ...
    def _dataframe_to_workbook(df):
        ...
        if isinstance(df, dict) and len(df) > 1:
            return DeepDocExcelParser._dataframes_to_workbook(df)

        df = DeepDocExcelParser._clean_dataframe(df)
        wb = Workbook()
        ws = wb.active
        ws.title = "Data"
    ...
    def _get_rows_limited(ws):
        actual_rows = DeepDocExcelParser._get_actual_row_count(ws)
        if actual_rows == 0:
            return []
    ...
    def html(self, fnm, chunk_rows=256):
        ...
        file_like_object = DeepDocExcelParser._ensure_file_like(fnm)
        wb = DeepDocExcelParser._load_excel_to_workbook(file_like_object)
        tb_chunks = []
        ...
        for sheetname in wb.sheetnames:
            ws = wb[sheetname]
            try:
                rows = DeepDocExcelParser._get_rows_limited(ws)
            except Exception as e:
                logger.warning(
                    f"Skip sheet '{sheetname}' due to rows access error: {e}"
                )
                continue
            ...
            for chunk_i in range((len(rows) - 1) // chunk_rows + 1):
                ...
                for r in list(
                    rows[
                        1
                        + chunk_i
                        * chunk_rows : min(1 + (chunk_i + 1) * chunk_rows, len(rows))
                    ]
                ):
                    tb += "<tr>"
    ...
    def markdown(self, fnm):
        ...
        try:
            ...
            logger.warning(
                f"Parse spreadsheet error: {e}, trying to interpret as CSV file"
            )
            file_like_object.seek(0)
            df = pd.read_csv(file_like_object, on_bad_lines="skip")
    ...
    def row_number(fnm, binary):
        if fnm.split(".")[-1].lower().find("xls") >= 0:
            wb = DeepDocExcelParser._load_excel_to_workbook(BytesIO(binary))
            total = 0

            for sheetname in wb.sheetnames:
                try:
                    ws = wb[sheetname]
                    total += DeepDocExcelParser._get_actual_row_count(ws)
                except Exception as e:
                    logger.warning(
                        f"Skip sheet '{sheetname}' due to rows access error: {e}"
                    )
